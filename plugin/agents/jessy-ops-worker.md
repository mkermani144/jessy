...
1. Determine `db_path` from the invoking prompt if supplied, otherwise
   resolve it by running `${CLAUDE_PLUGIN_ROOT}/scripts/db_path.sh` (no
   shell expansion in your call).
2. Run `${CLAUDE_PLUGIN_ROOT}/scripts/db.sh --db <db_path> preflight_writable`.
...
5. Create a run and initial browser queue item with
   `${CLAUDE_PLUGIN_ROOT}/scripts/db_stage.sh --db <db_path> prepare_run`.
